...
#
# Note: This script has been derived from a similar script in MediaPipe's repo.
#
# Script to setup Android SDK, Build tools, Platform tools and NDK.
# usage:
# $ cd <root dir>
# $ bash ./setup_android_build_env.sh ~/Android/Sdk ~/Android/Ndk r21e [--accept-licenses]
...
if [ -z $3 ]
then
  echo "Warning: ndk_version (argument 3) is not specified. Fallback to r21e."
  ndk_version="r21e"
fi
...
sed -i '/^android_[sn]dk_repository/d' $workspace_file
echo "android_sdk_repository(name = \"androidsdk\", path = \"${android_sdk_path}\")" >> $workspace_file
echo "android_ndk_repository(name = \"androidndk\", api_level=30, path = \"${android_ndk_path}/android-ndk-${ndk_version}\")" >> $workspace_file
echo "Done"
